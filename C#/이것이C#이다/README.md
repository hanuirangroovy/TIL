...
- 파생 클래스는 객체를 생성할 때 기반 클래스의 생성자를 호출한 후에 자신의 생성자를 호출하고, 객체가 소멸될 때는 반대의 순서로(파생클래스 -> 기반 클래스) 종료자를 호출합니다.
...
- 구조체는 값 형식이기 때문에 할당 연산자 =를 통해 모든 필드가 그대로 복사 됨
...
- 객체의 상태가 늘어나고 프로그램 논리가 복잡해지면 건드려서는 안되는 상태를 수정하게 되는 실수를 종종 저지를 수 있는데 readonly 한정자를 이용해서 메소드에게 상태를 바꾸지 않도록 강제하면 이런 문제를 해결할 수 있음
...
- 형식 이름이 없기에 응용 프로그램 전체에서 사용할 형식을 선언할 때가 아닌, 즉석에서 사용할 복합 데이터 형식을 선언할 때 적합

- 구조체이므로 값 형식

  - 값 형식 : 생성된 지역을 벗어나면 스택에서 소멸되기 때문에 프로그램에 장기적인 부담을 주지 않는 장점 있음
...
- 기본적인 튜플 형식
  ...
  - 필드의 이름을 지정하지 않는 튜플을 일컬어 "명명되지 않은 튜플"이라 부름

- 필드의 이름을 지정할 수 있는 명명된 튜플

  - "필드명:"의 꼴로 필드의 이름을 지정하여 선언

- 튜플을 분해할 때 특정 필드를 무시하고 싶다면 _를 이용

- 튜플 분해를 이용하면 여러 변수를 단번에 생성하고 초기화할 수 있음

- 명명되지 않은 튜플과 명명된 튜플끼리는 필드의 수와 형식이 같으면 할당이 가능

- 튜플이 분해가 가능한 이유는 분해자를 구현하고 있기 때문
  - 분해자를 구현하고 있는 객체를 분해한 결과를 switch 문이나 switch 식의 분기 조건에 활용할 수 있음. 이를 패턴매칭이라고 함.
  - 식별자나 데이터 형식이 아닌 분해된 요소의 위치에 따라 값이 일치하는지를 판단하는 것.
...
1. 클래스와 객체, 인스턴스
   ...
   - 클래스 - 객체가 가지게 될 속성과 기능을 정의하지만 실체를 가지지 않음. 
   - 객체 - 객체는 속성과 기능을 가짐. 고유한 실체를 가지며 저마다 메모리 공간을 차지함.
   - 인스턴스 - 객체를 인스턴스라고 부르기도 함.
...
2. ```
   ...
   class C
   {
   	public static void Main()
   	{
   		...
   		B b = new B();
   		A c = new B();
   		B d = new A(); // 파생 클래스는 기반 클래스를 참조할 수 없음
   	}
   }
   ```

3. this 키워드와 base 키워드

   - this 키워드 - 자기 자신을 가리킴. 객체가 자신을 지칭할 때 사용하는 키워드
   - base 키워드 - 기반 클래스를 가리킴. 기반클래스의 멤버에 접근할 수 있음
...
- 이 장의 핵심개념
  - 인터페이스가 무엇인지 파악합니다.
  - 인터페이스로부터 상속하는 방법을 이해합니다.
  - 추상 클래스가 무엇인지 파악하고, 인터페이스와의 차이를 이해합니다.



#### 8.1 인터페이스의 선언

- 인터페이스는 메소드, 이벤트, 인덱서, 프로퍼티만 가질 수 있는데 구현부가 없음
- 접근 제한 한정자를 사용할 수 없고 모든 것이 public으로 선언됨.
- 인터페이스는 인스턴스를 가질 수 없지만, 이 인터페이스를 상속받는 클래스의 인스턴스를 만드는 것은 가능.
  - 파생클래스는 인터페이스에 선언된 모든 메소드를 구현해줘야 하며 이 메소드들은 public 한정자로 수식해야 함.
- 인터페이스는 인스턴스를 못 만들지만 참조는 만들 수 있음
  - 참조에 파생 클래스의 객체의 위치를 담음
- 대개 인터페이스의 이름 앞에 I를 붙이는 것이 관례



#### 8.2 인터페이스는 약속이다

- 인터페이스는 클래스가 따라야 하는 약속이 되는데 이 약속은 인터페이스로부터 파생될 클래스가 어떤 메소드를 구현해야 할지를 정의



#### 8.3 인터페이스를 상속하는 인터페이스

- 기존의 인터페이스에 새로운 기능을 추가한 인터페이스를 만들고 싶을 때 인터페이스를 상속하는 인터페이스를 만들면 됨.

- 인터페이스를 수정할 수 없을 때는 인터페이스를 상속하는 인터페이스를 이용해야 함.

  - 상속하려는 인터페이스가 소스 코드가 아닌 어셈블리로만 제공되는 경우
    - .NET SDK에서 제공하는 인터페이스들이 그 예
    - 어셈블리 안에 있기 때문에 인터페이스를 수정할 수 없음
    - 인터페이스에 새로운 기능을 추가한 인터페이스를 만들고 싶으면 상속하는 수밖에 없음
  - 상속하려는 인터페이스의 소스 코드를 갖고 있어도 이미 인터페이스를 상속하는 클래스들이 존재하는 경우
    - 클래스는 반드시 인터페이스의 모든 메소드와 프로퍼티를 구현해야 함
    - 인터페이스에 사소한 수정이라도 이루어지면 인터페이스를 상속하는 기존 클래스들은 소스 코드를 빌드할 때 컴파일 에러가 발생
    - 이런 상황에서 기존의 소스코드에 영향을 주지 않고 새로운 기능을 추가하기 위해서는 인터페이스를 상속하는 인터페이스를 이용하는 편이 좋음

- 인터페이스가 인터페이스를 상속하기 위해 사용하는 문법은 클래스의 문법과 같은데 파생 인터페이스의 이름 옆에 ':'을 찍고 그 오른편에 상속할 인터페이스의 이름을 붙여주면 됨
...
- 파생 인터페이스는 기반 인터페이스에 선언된 모든 것을 그대로 물려받음
...
- 인터페이스는 내용이 아닌 외형을 물려주기에 죽음의 다이아몬드같은 문제가 발생하지 않음. 그래서 여러 인터페이스를 다중 상속하는 클래스를 안심하고 이용할 수 있음

- 여러 클래스로부터 구현을 물려받고 싶다면

  - 다른 클래스의 기능을 새로운 클래스에 넣는 방법에 꼭 상속만 있는 것은 아님

  - 포함이라는 기법이 있음. 클래스 안에 물려받고 싶은 기능을 가진 클래스들을 필드로 선언해 넣는 것
...
- 추상 클래스는 구현을 가질 수 있으나 인스턴스는 만들지 못함

- 추상 클래스 선언

  - ```
    abstract class 클래스이름 // abstract 한정자와 class 키워드를 이용해 선언
    {
    	// 클래스와 동일하게 구현
    }
    ```

- 인터페이스에서는 모든 메소드가 public으로 선언되는 반면, 클래스는 한정자를 명시하지 않으면 모든 메소드가 private로 선언되기에 추상 클래스의 접근성을 보면 클래스에 더 가까움

- 추상 클래스는 추상 메소드를 가질 수 있음

  - 추상 메소드 - 추상 클래스가 인터페이스의 역할도 할 수 있게 해주는 장치
  - 구현을 갖지는 못하지만 파생 클래스에서 반드시 구현하도록 강제
  - 추상 클래스를 상속하는 클래스들이 반드시 이 메소드를 갖고 있을 거라는 약속

- 추상 클래스나 클래스는 그 안에서 선언되는 모든 필드, 메소드, 프로퍼티, 이벤트 모두 접근 한정자를 명시하지 않으면 private. 

  - 추상 메소드도 예외는 X.

  - C# 컴파일러는 추상 메소드가 반드시 public, protected, internal, protected internal 한정자 중 하나로 수식될 것을 강요

  - 이렇게 하면 클래스의 접근성 원칙도, 인터페이스의 접근성 원칙도 지켜질 수 있음.
...
- 추상 클래스가 또 다른 추상클래스를 상속하는 경우

  - 추상 클래스는 또 다른 추상 클래스를 상속할 수 있으며, 이 경우 자식 추상클래스는 부모 추상 클래스의 추상 메소드를 구현하지 않아도 됨.
  - 추상 메소드는 인스턴스를 생성할 클래스에서 구현하면 되기에

- 추상 클래스는 파생 클래스를 만들어야 하며 모든 추상 메소드를 구현해야 한다는 사실을 잊어버린다 해도 컴파일러가 상기시켜 주기에 추상 클래스를 이용함
...
1. 인터페이스와 클래스
   - 인터페이스 - 실체. 메소드, 이벤트, 프로퍼티, 인덱서만 선언 가능. 다중 상속 가능. 
   - 클래스 - 청사진. 메소드를 구현할 수 있음. 여러 클래스를 한꺼번에 상속할 수 없음
2. 인터페이스와 추상 클래스
   - 인터페이스 - 모든 메소드가 public으로 선언
   - 추상 클래스 - 데이터를 선언할 수 있음. 클래스와 같이 그 안에서 선언되는 모든 필드, 케소드, 프로퍼티, 이벤트 모두 접근 한정자를 명시하지 않으면 모든 메소드가 private로 선언
...
- 이 장의 핵심개념
  - 프로퍼티가 무엇인지 이해합니다.
  - 메소드, 필드, 프로퍼티 간의 차이를 이해합니다.
  - 프로퍼티를 이용하여 객체를 초기화하는 방법을 익힙니다.
  - 무명 형식을 이해합니다.
  - 인터페이스와 추상 클래스에서의 프로퍼티 선언 방법을 익힙니다.



#### 9.1 public 필드의 유혹

- 프로퍼티를 이용하면 은닉성과 편의성을 다 잡을 수 있음.
...
- 프로퍼티 선언하는 문법
  ...
  - get 접근자는 필드로부터 값을 읽어오고 set 접근자는 필드에 값을 할당함

  - set 접근자 안에 있는 value 키워드는 선언한 적이 없지만 C# 컴파일러가 SET 접근자의 암묵적 매개변수로 간주하므로 문제가 삼지 않음

- set 접근자를 구현하지 않으면 해당 프로퍼티는 쓰기 불가한 읽기 전용이 됨

  - ```
    class MyClass
    {
    	private int myField;
    	public int MyField // 이렇게 get 접근자만 있으면 읽기 전용 프로퍼티가 됨.
    	{
    		get
    		{
    			return myField;
...
- 프로퍼티는 데이터의 오염에 대해선 메소드처럼 안전하고, 데이터를 다룰 때는 필드처럼 간결
- C#7.0부터는 저동 구현 프로퍼티를 선언함과 동시에 초기화를 수행할 수 있어서 자동 구현 프로퍼티에 초깃값이 필요할 떄 생성자에 초기화 코드를 작성하는 수고를 덜게 되었음.
...
- 프로퍼티를 이용한 초기화
  ...
  - ```
    클래스이름 인스턴스 = new 클래스이름()
    	{
    		프로퍼티1 = 값,
    		프로퍼티2 = 값, // 세미콜론이 아니라 콤마(,)
    		프로퍼티3 = 값
    	};
    ```

- 객체를 생성할 때 <프로퍼티 = 값> 목록에 객체의 모든 프로퍼티가 올 필요는 없음.
...
- 불변 객체 - 내부 상태(데이터)를 변경할 수 없는 객체
- 값 형식 객체는 다른 객체에 할당할 때 깊은 복사를 수행
  - 깊은 복사 - 모든 필드를 새 객체가 가진 피륻에 1:1로 복사하는 것
  - 배열의 요소에 입력하거나 함수 인수로 사용할 때도 깊은 복사를 함
  - 필드가 많으면 많을수록 복사 비용은 커짐. 객체를 여러 곳에서 사용해야 하는 경우에도 커짐.
    - 참조는 객체가 차조하는 메모리 주소만 복사하면 되기에 이런 오버헤드가 없음.
    - 참조 형식은 직접 깊은 복사를 구현해야 함
-  값 형식은 객체를 비교할 때 모든 필드를 1:1로 비교
  - 참조는 참조 형식끼리 내용 비교를 할 수 있으려면 직접 비교 코드를 작성해야 함.
  - 보통 object로부터 상속하는 Equals() 메소드를 오버라이딩함.
- 불변 객체를 참조 형식으로 선언하면 함수 호출 인수나 컬렉션 요소로 사용할 때 복사 비용을 줄일 수 있음
- 불변 객체는 새 상태 표현과 상태 확인을 위해 깊은 복사와 내용 비교가 필수적이므로 값 형식으로 선언하는 편이 편리한 부분이 많음
- 레코드 형식은 값 형식처럼 다룰 수 있는 불변 참조 형식으로 참조 형식의 비용 효유로가 값 형식이 주는 편리함을 모두 제공



##### 9.6.1 레코드 선언하기

- 레코드는 record 키워드와 초기화 전용 구현 프로퍼티를 함께 이용해서 선언

- 레코드에는 초기화 전용 자동 구현 프로퍼티뿐만 아니라 쓰기 가능한 프로퍼티와 필드도 자유롭게 선언해 넣을 수 있음

- 초기화 전용 자동 구현 프로퍼티만을 이용해서 상태 표현

  - ```
    record RTransaction
    {
    	public string Form {get; init;}
    	pubic string To {get; init;}
    	public int Amount {get; init;}
    }
    ```
...
- 형식의 이름을 이용해서 인스턴스를 만들기 때문에 형식의 이름 필요

- 무명 형식은 형식의 선언과 동시에 인스턴스를 할당

  - 인스턴스를 만들고 다시는 사용하지 않을 때 무명ㅎ 셩식이 요긴함
  - 두 개 이상의 인스턴스를 만들려면 class나 struct를 이용해 이름을 가진 형식을 별도로 만들어야 함

- ```
  var myInstance = new {Name="박상현", Age="17"}; 
  ```

  - 괄호 {와 } 사이에 임의의 프로퍼티 이름을 적고 값을 할당하면 그대로 새 형식의 프로퍼티가 됨.

  - 이와 같이 선언한 무명 형식의 인스턴스는 여느 객체처럼 프로퍼티에 접근하여 사용할 수 있음.
...
- 주의점
  ...
  - 무명 형식의 프로퍼티에 할당된 값은 변경불가능
  - 무명 형식의 인스턴스가 만들어지고 난 다음에는 읽기만 할 수 있음
...
- 프로퍼티나 인덱서를 가진 인터페이스를 상속하는 클래스가 반드시 해당 프로퍼티와 인덱서를 구현해야 함

- 인터페이스에 들어가는 프로퍼티는 구현을 갖지 않음

- 여기서 한 가지 문제는 인터페이스의 프로퍼티 선언은 클래스의 자동 구현 프로퍼터ㅣ 선언과 그 모습이 동일하다는 것
...
- 프로퍼티를 가진 인터페이스와 이를 상속하는 파생 클래스
  ...
  - ```
    ...
    class Product : IProduct
    {
    	...
    	public string ProductName // 파생클래스는 기반 인터페이스에 선언된 모든 프로퍼티를 구현해야 함.
    	{
...
- 배열이 무엇인지 이해합니다.
- 배열을 선언하고 초기화하는 방법을 익힙니다.
- System.Array 클래스를 이용해 배열을 다루는 사용 방법을 익힙니다.
- 여러 종류의 배열을 사용하는 방법을 익힙니다.
- 다양한 컬렉션 클래스를 이해합니다.
- 인덱서를 선언하고 사용하는 방법을 익힙니다.



#### 10.1 All for one, one for all

- 배열은 변수의 개수를 늘려 선언할 수만은 없을 때 이를 해결해줌.
- 배열 선언
  - [와]가 포인트
  - `데이터형식[] 배열이름 = new 데이터형식[용량];`
  - ex. `int[] scores = new int[5];`
- 배열의 각 요소에 데이터를 저장하거나, 요소 안에 있는 데이터를 읽어올 때는 배열 이름 뒤에 괄호 [와]를 붙여주고, 괄호 사이에 인덱스를 적어주면 됨.
  - 인덱스는 0부터 시작 

- System.Index 형식과 ^ dustkswk

  - ^ 연산자 - 컬렉션의 마지막부터 역순으로 인덱스를 지정

    - ^1 - 컬렉션의 마지막 요소
    - ^2 - 마지막에서 두번째

  - ^ 연산자의 연산 결과는 System.Index 형식의 인스턴스로 나타남.

  - ```
    System.Index last = ^1;
    scores[last] = 36; // scores[scores.Length-1] = 34;와 동일
    ```
...
- 컬렉션 초기자
  ...
  - 배열의 원소 개수를 명시하고 그 뒤에 중괄호 {}로 둘러싸인 블록을 붙인 뒤, 블록 사이에 배열의 각 원소에 입력될 데이터 입력
...
- 첫 번째 방법에서 배열의 용량을 생략하는 것

  - ```
    string[] array2 = new string[]{"안녕", "Hello","Halo"};
    ```

- new 연산자, 형식과 대괄호 []와 배열의 용량을 모두 생략한 채 코드 블록 사이에 배열의 각 원소에 할당할 데이터를 넣어주기
